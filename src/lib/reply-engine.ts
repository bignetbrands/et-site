...
export async function decideReply(input: ReplyInput): Promise<ReplyDecision> {
  ...
  // ── 3. FACE SWAP — parent/mention has a photo ──────────────────────────────
  const photoUrl = imageUrls?.[0] || (!hasVideo ? parentImageUrls?.[0] : undefined);
  const shouldFaceSwap = (
    !isFinancialAdvisorMention(tweetText) &&
    !!photoUrl &&
    !hasVideo &&
    Math.random() < 0.4 // 40% chance — keeps it special
  );

  if (shouldFaceSwap && photoUrl) {
    try {
